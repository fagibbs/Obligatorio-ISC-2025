# ID de la VPC principal exportado para usar en los otros módulos
output "vpc_id" {
  description = "ID de la VPC principal"
  value       = aws_vpc.main.id
}

# Lista con las subnets públicas
output "public_subnets" {
  description = "Subnets públicas (ALB y NAT)"
  value       = [aws_subnet.public_a.id, aws_subnet.public_b.id]
}

# Lista con las subnets privadas de aplicación para el Auto Scaling Group
output "app_subnets" {
  description = "Subnets privadas para servidores de aplicación"
  value       = [aws_subnet.app_a.id, aws_subnet.app_b.id]
}

# Lista con las subnets privadas de DB
output "db_subnets" {
